...
import { TELEMETRY_LAYERS, TelemetryMiddlewareService } from '@inh-lib/unified-telemetry-middleware';
import { OtelProviderService } from '@inh-lib/unified-telemetry-otel';
import type { UnifiedTelemetryProvider,UnifiedTelemetrySpan } from '@inh-lib/unified-telemetry-core';
...
import {  createLogicProcessor, createServiceQueryProcessor } from './processor';
...
declare module 'fastify' {
    ...
    interface FastifyRequest {
        ...
        businessLogicContext?: UnifiedHttpContext; // For business logic layers
    }
}
// Simple data models
interface User {
 ...
 */
function createRealTelemetryProvider(fastify: FastifyInstance): UnifiedTelemetryProvider {
    ...
    const provider = OtelProviderService.createProvider(
       ...
       new FastifyTelemetryLoggerAdapter(fastify.log), // Use Fastify logger for OTEL
...
class UserService {
    ...
    async listAll(context: UnifiedHttpContext): Promise<User[]> {
        
        const logger = this.telemetryMiddleware.getCurrentLogger(context);
        const fn = (context: UnifiedHttpContext) => this.repository.findAll(context);

        logger.info('creating repository.findAll processer');
        const proc = createLogicProcessor(this.telemetryMiddleware, fn, 'user-api.listAll',TELEMETRY_LAYERS.SERVICE);
        logger.info('created repository.findAll processer');

        logger.info('Processing repository.findAll processer');
        const users = await proc.process(context);
        logger.setSpanAttribute('users.count', users.length);
        logger.setSpanAttribute('operation.success', true);
    ...
    async findAll(context: UnifiedHttpContext): Promise<User[]> {
        ...
        const { span, logger, finish } = this.telemetryMiddleware.createActiveSpan(
            context,
...
class HybridTelemetryApp {
    ...
    async initialize(): Promise<void> {
        try {
            ...
            this.telemetryProvider = createRealTelemetryProvider(this.app);
    ...
    private setupRoutes(): void {
        // ✅ Proper middleware pattern for GET /api/compose
        this.app.get('/api/compose', async (request, reply) => {
            let context: UnifiedHttpContext = request.unifiedAppContext
            if (!request.unifiedAppContext) {
                console.log('create new unifiedAppContext');
                context = createUnifiedContext(request, reply);
            }
          
            // Apply telemetry middleware and execute business logic

         const apiProcess = this.telemetryMiddleware.createOperationLayerProcess('api.process',
            {
            ...
            const apiLogic1Step = this.telemetryMiddleware.createApiMiddlewareStep('api.process.logic1Step',)

            const apiLogic1StepHandler = async (context, next: () => Promise<void>) => {
                const logger = this.telemetryMiddleware.getCurrentLogger(context);
                logger?.info('processing apiLogic1Step');

                await next();
            }


            const apiLogic2Step = async (context, next: () => Promise<void>) => {
                // implement apiLogic2StepHanlder
                 const { logger, finish } = this.telemetryMiddleware.createChildSpan(context, 'api.process.logic2Step', {
                        operationType: 'middleware',
                        layer: 'api',
                        attributes: {
                            'http.method': 'GET',
                            'http.route': '/api/compose',
                            'middleware.name': 'apiLogic2Step'
                        }
                    });
                    logger.info('Starting apiLogic2Step')

                try {

                   
                    logger?.info('processing apiLogic2Step');

                    await next();
                } finally {
                    logger.info('end apiLogic2Step')
                    finish(); // Ensure span is finished
                }

            }

            // const apiEndpointStep = this.telemetryMiddleware.createOperationLayerProcess('api.process.listAll', {

            //     operationType: 'endpoint',
            //     layer: 'api'
            // });

            const apiRouteHandler = async (context: UnifiedHttpContext) => {

                const logger = this.telemetryMiddleware.getCurrentLogger(context);

                logger.info('creating userService.listAll processer');
                //  need arrow function to bind this
                const fn = (context: UnifiedHttpContext) => this.userService.listAll(context);
                const proc = createServiceQueryProcessor(this.telemetryMiddleware, fn, 'api.process.listAll');
                logger.info('created userService.listAll processer');
 
                logger.info('Processing userService.listAll processer');
                const users = await proc.process(context);
            ...
            const apiPipe: UnifiedMiddleware[] = [apiProcess, apiLogic1Step, apiLogic1StepHandler, apiLogic2Step];
            // const endpointPipe: UnifiedMiddleware[] = [apiEndpointStep ];
            // const endPointHandler = composeMiddleware(endpointPipe)(apiEndpointStepHandler);
            const routeHandler = composeMiddleware(apiPipe)(apiRouteHandler);
        ...
        //     // Apply telemetry middleware and execute business logic
        //     const telemetryMiddleware = this.telemetryMiddleware.createBusinessLogicMiddleware( 'endpoint.listusers',
        //                 {
        //                     operationType: 'http',
        //                     layer: 'http',
        //                     attributes: {
        ...
        //         const logger =  this.telemetryMiddleware.getCurrentLogger(request.unifiedAppContext);
        //         const activeSpan = this.telemetryProvider.tracer.getActiveSpan();
        ...
        //         logger.info('TelemetryPluginService improved APIs investigation', {
        ...
        //     try {
        //         const mw: TelemetryMiddlewareService = this.app.telemetry?.unifiedTelemetryService;
        //         const init = createUnifiedContext(request, reply);
        //         // Use new clear method name - createRootSpan instead of misleading createChildSpan
        //         const initContext = mw.initializeContext(init);

            

        //         const rootSpan = mw.getCurrentSpan(initContext);
        ...
        //             const activeSpan = this.app.telemetry?.provider.tracer.getActiveSpan();
        ...
        //    const context = request.unifiedAppContext;
        ...
        //     const currentSpan = TelemetryRequestUtils.getRequestSpan(request) as UnifiedTelemetrySpan;
